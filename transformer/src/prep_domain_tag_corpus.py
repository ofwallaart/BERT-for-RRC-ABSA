...
def write_text(fn, data, skip_tag=True):
    with open(fn, "w", encoding='utf8') as fw:
...
def yelp(keep_probs, with_test, train_data, test_data):
    ...
    with open("../data/pt/yelp/business.json", encoding='utf8') as f:
        for l in f:
            rec = json.loads(l)
    ...
    with open("../data/pt/yelp/review.json", encoding='utf8') as f:
        for l in f:
            rec = json.loads(l)
...
def main(keep_probs=0.8, with_test=True):
    ...
    write_text("../data/pt/domain_v2_train.txt", train_data, skip_tag=True)
    write_text("../data/pt/domain_v2_dev.txt", test_data[:split_test_as_dev2], skip_tag=True)
    if with_test:
        write_text("../data/pt/domain_v2_test.txt", test_data[split_test_as_dev2:], skip_tag=True)
